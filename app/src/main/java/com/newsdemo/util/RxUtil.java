...
public class RxUtil {
                ...
                return httpResponseFlowable.flatMap(new Function<GankHttpResponse<T>, Flowable<T>>() {
                    ...
                    public Flowable<T> apply(GankHttpResponse<T> tGankHttpResponse){
                        if (!tGankHttpResponse.getError()){
                            ...
                            return Flowable.error(new APIException("SERVER RETURN ERROR"));
                        }
                    }
                });
            };
    ...
    public static <T> FlowableTransformer<WXHttpResponse<T>,T> handleEXResult(){
        return new FlowableTransformer<WXHttpResponse<T>, T>() {
            ...
            public Flowable<T> apply(Flowable<WXHttpResponse<T>> wxHttpResponse) {
              return wxHttpResponse.flatMap(new Function<WXHttpResponse<T>, Flowable<T>>() {
                  @Override
    ...
    public static <T> FlowableTransformer<MyHttpResponse<T>,T> handleMyResult(){//compose判断结果
        return new FlowableTransformer<MyHttpResponse<T>, T>() {
            @Override
            public Flowable<T> apply(Flowable<MyHttpResponse<T>> myHttpResponseFlowable) {
                return myHttpResponseFlowable.flatMap(new Function<MyHttpResponse<T>, Flowable<T>>() {
                    @Override
    ...
    public static <T> FlowableTransformer<GoldHttpResponse<T>,T> handleGoldResult(){ //compose判断结果
        return new FlowableTransformer<GoldHttpResponse<T>, T>() {
            @Override
            public Flowable<T> apply(Flowable<GoldHttpResponse<T>> goldHttpResponseFlowable) {
                return goldHttpResponseFlowable.flatMap(new Function<GoldHttpResponse<T>, Flowable<T>>() {
                    @Override
                    public Flowable<T> apply(GoldHttpResponse<T> tGoldHttpResponse) throws Exception {
